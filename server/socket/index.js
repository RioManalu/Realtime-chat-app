...
io.on("connection", async (socket) => {
    ...
    onlineUser.add(user?._id?.toString());      //bisa tambahkan toString()
    ...
    socket.on("new message", async (data) => {
        ...
        const updataConversation = await ConversationModel.updateOne({ _id : conversation._id }, {
    ...
    socket.on("seen", async(msgByUserId) => {
        ...
        const updateMessages = await MessageModel.updateMany(
            { id : { "$in" : conversationMessageId }, msgByUserId : msgByUserId },
            { "$set" : { seen : true } }
        );
